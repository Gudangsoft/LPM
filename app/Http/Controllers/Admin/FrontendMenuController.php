...
/**
 * Drag-and-drop editor for the public website navbar (menus with lokasi = frontend).
 */
class FrontendMenuController extends Controller
{
    private const LOCATION = 'frontend';
    private const ROUTE_PREFIX = 'admin.menu-web';

    private function query()
    {
        return Menu::where('lokasi', self::LOCATION);
    }

    private function route(string $action, $param = []): string
    {
        return route(self::ROUTE_PREFIX . '.' . $action, $param);
    }
    ...
    public function index()
    {
        ...
        return view('admin.menu-web.index', [
            'tree' => Menu::editorTree(self::LOCATION),
            'parentOptions' => $parentOptions,
            'routePrefix' => self::ROUTE_PREFIX,
            'pageTitle' => __('admin.website_menu_editor'),
    ...
    {
        return view('admin.menu-web.create', [
    ...
    public function store(Request $request)
    {
        ...
        $validated['lokasi'] = self::LOCATION;
    ...
    public function edit(Menu $menu)
    {
        ...
        return view('admin.menu-web.edit', [
            'menu' => $menu,
    ...
    public function quickUpdate(Request $request, Menu $menu)
    {
        ...
        if ($request->has('target')) {
            ...
            if ($target === '') {
                $menu->route = null;
                $menu->url = null;
            } elseif (Str::startsWith($target, ['http://', 'https://', '/', '#', 'mailto:', 'tel:'])) {
                $menu->url = $target;
                $menu->route = null;
            } else {
                $menu->route = $target;
    ...
    private function assertOwned(Menu $menu): void
    {
        abort_unless($menu->lokasi === self::LOCATION, 404);
    }

    private function assertParentSameLocation(?int $parentId): void
    {
        if ($parentId) {
            abort_unless(Menu::whereKey($parentId)->value('lokasi') === self::LOCATION, 422);
        }
    }
