...
@section('content')
    <div class="page-header">
        <h1 class="page-title">Tambah Akreditasi</h1>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('prodi_id') is-invalid @enderror" id="prodi_id" name="prodi_id" required>
                            ...
                            <option value="{{ $prodi->id }}" {{ old('prodi_id', request('prodi_id')) == $prodi->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('lembaga') is-invalid @enderror" id="lembaga" name="lembaga" required>
                            ...
                            <option value="{{ $l }}" {{ old('lembaga') == $l ? 'selected' : '' }}>{{ $l }}</option>
                            @endforeach
                        </select>
                        @error('lembaga')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('peringkat') is-invalid @enderror" id="peringkat" name="peringkat" required>
                            ...
                            <option value="{{ $p }}" {{ old('peringkat') == $p ? 'selected' : '' }}>{{ $p }}</option>
                            @endforeach
                        </select>
                        @error('peringkat')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                            ...
                            <option value="proses_perpanjangan" {{ old('status') == 'proses_perpanjangan' ? 'selected' : '' }}>Proses Perpanjangan</option>
